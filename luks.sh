...
check_config BLK_DEV_LOOP
if [ $? -ne 0 ];then
	echo "DEBUG: Missing CONFIG_BLK_DEV_LOOP"
fi
...
if [ $? -ne 0 ];then
	echo "DEBUG: Missing CONFIG_CRYTPO_XTS"
fi
check_config CRYPTO_USER_API_SKCIPHER
if [ $? -ne 0 ];then
	echo "DEBUG: Missing CONFIG_CRYPTO_USER_API_SKCIPHER"
fi
check_config CRYPTO_USER_API_HASH
if [ $? -ne 0 ];then
